...
class AnimatedScatter(object):
    ...
    def __init__(self, numpoints=50):
        ...
        self.fig, self.ax = plt.subplots(nrows=1, ncols=1, figsize=(20, 20))
        # Then setup FuncAnimation.
        self.ani = animation.FuncAnimation(self.fig, self.update, interval=5, 
                                          init_func=self.setup_plot, blit=True, save_count=399)
    ...
    def grid_plot(self):
        ...
        for n in range(0, L[0]-1, 1):
            for i in range(L[0]-1):
                ...
                xs, ys = zip(*coord)
                self.ax.plot(xs, ys, 'r-', alpha=0.6, linewidth=0.2)

        self.ax.plot([xl[L[0]-1], xl[2*L[0]-1]], [yl[L[0]-1], yl[2*L[0]-1]], 'r-', alpha=0.6, linewidth=0.2)
        self.ax.plot([xl[2*L[0]*(L[0]-1)], xl[2*L[0]*L[0] - L[0]]], [yl[2*L[0]*(L[0]-1)], yl[2*L[0]*L[0] - L[0]]], 'r-', alpha=0.6, linewidth=0.2)
    ...
    def update(self, i):
        ...
        self.scat = self.ax.scatter(x=self.hex_cor(kos, jos)[0], y=self.hex_cor(kos, jos)[1], c=weight, cmap="Greens", s=s, vmin=0, vmax=n, marker="s")
        #self.grid_plot() # ! THIS LINE TAKES FOREVER
        self.ax.axis([-90, 180, -3, 150])
        self.ax.set_title(r"$N=9000, \alpha=0.05, n_\mathrm{max}=3$")
...
if __name__ == '__main__':
    a = AnimatedScatter()
    a.ani.save('Hex_alp_05_n_3_N_9000.gif', fps=10)
    #a.update(0)
    #plt.show()
