...
class AnimatedScatter(object):
    ...
    def __init__(self, numpoints=50):
        ...
        f = open("./Data/weighted/hex_part.txt")
        self.data = []
        for line in f:
            self.data.append(np.loadtxt(StringIO(line), dtype=int))
        
        self.numb = np.genfromtxt("./Data/weighted/hex.txt", delimiter=" ")
...
if True:
    ...
    def animate_tri(i):
        ...
        ax[1].axis([min(numb[0::2, 0]), max(numb[0::2, 0]), min(numb[0::2, 1]), max(numb[0::2, 1])])
        ax[1].set_yscale("log")
        #ax[1].set_xscale("log")
...
if False:
    ...
    numb = np.genfromtxt("./Data/weighted/tri.txt", delimiter=" ")
    ...
    # Then setup FuncAnimation.
    ani_tri = animation.FuncAnimation(fig, animate_tri, interval=1, blit=False, save_count=len(numb)-1)
    ani_tri.save('./gif/tri_n_1_alp_0.01_long.gif', writer='PillowWriter', fps=10)
...
if __name__ == '__main__':
    #a = AnimatedScatter()
    #a.ani.save('./gif/hex_n_1_alpha_0.001_longs.gif', fps=10)
    #a.update(0)
    #plt.show()
